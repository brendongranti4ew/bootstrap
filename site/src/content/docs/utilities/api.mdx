...
```scss
// my-bootstrap.scss — must be defined before the $utilities map is built
@use "bootstrap/scss/utilities" as bs-utils;

$utilities: map.merge(
  bs-utils.$utilities,
  (
    "width": map.merge(
      map.get(bs-utils.$utilities, "width"),
      (
        values: map.merge(
          map.get(map.get(bs-utils.$utilities, "width"), "values"),
          (10: 10%),
        ),
      ),
    ),
...
$utilities: map.merge(
  ...
  (
    "border": map.merge(
      map.get(bs-utils.$utilities, "border"),
      ( responsive: true ),
    ),
  )
);
...
$utilities: map.merge(
  ...
  (
    "margin-start": map.merge(
      map.get(bs-utils.$utilities, "margin-start"),
      ( class: ml ),
    ),
  )
);
...
$utilities: map.remove(bs-utils.$utilities, "width", "float");
